...
// Let the number of vertices in the graph equal to 0 at first.
int V = 0;
...
{
#pragma omp for // do the matrix multiplication in parallel
    for (int i = 0; i < V; i++)
    {
        for (int j = 0; j < V; j++)
        {
...
{
    int **aux2 = new int*[V];  // int aux2[V][V] to store graph2
    int **aux3 = new int*[V];  // int aux3[V][V] to store graph3
